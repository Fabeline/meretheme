const getPageId = () => {
    ...
    const pageFromPath = document.location.pathname.split("/").pop().replace(".html", "");

    const pagefromHistory = window.history.state;

    /*console.log("page from url", pageFromUrl);
    console.log("page from path", pageFromPath);
    console.log("page from history", pagefromHistory);*/
...
const activateInternalLinks = () => {
    ...
    menuLinks.forEach(link => {
        link.addEventListener('click', (event) => {
            event.preventDefault();
            const houseId = link.getAttribute('data-mt-link');
